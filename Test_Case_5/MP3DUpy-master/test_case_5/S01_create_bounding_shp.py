...
shps = os.listdir(os.path.join('output'))
shps = [item for item in shps if item.endswith('.shp')]

for shp in shps:
	shutil.copy('texas_gam.prj',os.path.join('output',shp.replace('.shp','.prj')))


gdf = gpd.read_file(os.path.join('output','Endpoint_mp3du.shp'))
...
gdf.crs = crs
gdf.to_file(os.path.join('output','mp3du_10_yrs_poly.shp'))
